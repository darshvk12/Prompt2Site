...
const Projects = () => {
    ...
    const saveProject = async () => {
        if(!project) return;
        setIsSaving(true);
        // simulate save and create a new version
        setTimeout(()=>{
            const newVersion = { id: Date.now().toString(), timestamp: new Date().toISOString(), code: project.current_code };
            setProject({...project, versions: [...project.versions, newVersion], current_version_index: newVersion.id});
            setIsSaving(false);
        }, 800);
    };
    ...
    const downloadCode = () =>{
        ...
        a.href = url;
        a.download = `${project.name.replace(/\s+/g, '_') || 'project'}.html`;
        ...
        <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
            ...
            <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
                ...
                <FullscreenIcon size={16}/>Preview
                    ...
                    <button onClick={togglePublish} className='bg-linear-to-br from-blue-700 to-blue-600 hover:from-blue-600
                    hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors'>
                     {project.isPublished ? 
